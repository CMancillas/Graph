...
template <typename T>
Graph<T>::Graph() : nodesSize(0), edgesSize(0),
    firstN(nullptr), lastN(nullptr){}
//**************************************************************************
// Metodo destructor
template <typename T>
Graph<T>::~Graph()
...
template <typename T>
Graph<T>::Graph(const Graph<T> & g) : nodesSize(0), edgesSize(0),
    firstN(nullptr), lastN(nullptr)
{
        *this = g;
}
...
// :)
template <typename T>
void Graph<T>::clearNode(T value)
{
    // Si no existe tal nodo se tira una excepcion
    if (!searchNode(value)) throw "No existe tal nodo";
...
template <typename T>
void Graph<T>::addEdge(T first, T last)
{
    Node *beginningPtr = getNodeAddress(first);
    if (beginningPtr == nullptr) return;

    Node *endPtr = getNodeAddress(last);
    if (endPtr == nullptr) return;

    if (searchEdge(first,last)) return;
    beginningPtr->addEdgeInNode(endPtr);
    endPtr->addEdgeInNode(beginningPtr);
    ++edgesSize;
...
// :)
template <typename T>
void Graph<T>::deleteEdge(T first, T last)
{
...
// :)
template <typename T>
bool Graph<T>::searchNode(T value) const
{
    // Nodo a verificar si existe
    Node *tmp = getNodeAddress(value);
...
// :)
template <typename T>
bool Graph<T>::searchEdge(T first, T last) const
{
    // Nodos a verificar si estan conectados
    Node *firstNode = getNodeAddress(first);
    Node *lastNode = getNodeAddress(last);

    if (firstNode == nullptr || lastNode == nullptr || firstNode->firstE == nullptr ||
        lastNode->firstE == nullptr) return false;

    // Puntero a arista a primer nodo que verificara si es adyacente con el otro nodo
    Edge *tmpE = firstNode->firstE;

    while (tmpE != nullptr)
    {
        // Si es adyacente regresa true
        if (tmpE->adjacent == lastNode) return true;
...
//:)
template <typename T>
void Graph<T>::printNode(T value) const
{
...
template <typename T>
void Graph<T>::Node::addEdgeInNode(Graph::Node *adj)
{
...
template <typename T>
void Graph<T>::Node::deleteNodesEdge(Graph::Node *adj)
{
    // Por borrar
